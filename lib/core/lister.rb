module CdnFu
  class Lister
    ...
    def list
      ...
      @globs.each do |glob|
        glob_str = glob[:name]
        Dir.glob(File.join(Config.config.asset_root_dir,glob_str)).each do |file|
          fi = FileInfo.new
          fi.local_path = File.expand_path(file)
          fi.gzip = glob[:gzip]
          fi.minify = glob[:minify]
          root_sub_path = fi.local_path.gsub(Config.config.asset_root_dir,'')
      ...
      @files.each do |file|
        ...
        fi.local_path = File.join(Config.config.asset_root_dir,file[:name])
        fi.gzip = file[:gzip]
